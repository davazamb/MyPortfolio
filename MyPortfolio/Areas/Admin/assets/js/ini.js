$(document).ready(function () {
    $("body").on('click', 'button', function () {
        ...
        //El metodo .data identifica la entrada y la castea al valor mas correcto
        ...
        if (button.data('confirm') != undefined) {
            if (button.data('confirm') == '') {
                if (!confirm('¿Esta seguro de realizar esta accion?')) return false;
            } else {
        ...
        //Creemos un div que bloqueara todo el formulario
        var block = $('<div class="block-loading"/>');
        form.prepend(block);
        //en caso de que haya habido un mensaje alerta
        $(".alert", form).remove();

        //Para los formularios que tengan Crupdate
        ...
        form.ajaxSubmit({
            ...
            success: function (r) {
                ...
                if (r.message != null) {
                    if (r.message.length > 0) {
                        ...
                        var message = '<div class="alert ' + css + ' alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button></div>';
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                block.remove();
                form.prepend('<div class="alert alert-warning alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">x</button></div>');
            }
        });
        return false;
    })
